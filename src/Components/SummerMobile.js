...
export default function SummerMobile() {
  const textRef = useRef(null);
  const gridRef = useRef(null);
  const containerRef = useRef(null);
  const places = useMemo(() => ['brazil', 'leh', 'monte', 'nz', 'yellowstone'], []);
  const ShowingPlaces = useMemo(() => ['RIO DE JANEIRO', 'LADAKH', 'MONTE CARLO', 'NEW ZEALAND', 'YELLOWSTONE'], []);
  ...
  const groupedImages = useMemo(() => {
    return places.map((name) => {
      ...
      formats.forEach((form) => {
        placeObj[form] = `/static/summer/${name}/${name}-${form}/${name}-${form}-medium/${name}1.${form}`;
      });
      return placeObj;
    });
  }, [places, formats]);
  ...
  }, [places, ShowingPlaces, groupedImages, daysAndNights]);
  ...
  return (
    <div ref={containerRef} className={styles.container}>
      <div className={styles.sectionName}>
        <p ref={textRef} className={styles.text}>SUMMER</p>
      </div>
