...
/**
 * SummerMobile component for mobile devices, displaying summer destinations with animations.
 * @param {Object} props - Component props
 * @param {string} [props.className] - Additional CSS class for the container
 */
const SummerMobile = forwardRef(({ className }, ref)=>{
  ...
  if (CONFIG.PLACES.length !== CONFIG.DAYS_NIGHTS.length || CONFIG.PLACES.length !== CONFIG.SHOWING_PLACES.length) {
    console.error("[SummerMobile] Mismatch between PLACES, SHOWING_PLACES, and DAYS_NIGHTS arrays");
  }
  ...
  const animateText = useCallback((chars) => {
    if (!chars?.length || !containerRef.current) {
      console.warn("[SummerMobile] Missing chars or containerRef for text animation");
      return;
    }
  ...
  const animateElements = useCallback((elements, options = {}) => {
    if (!elements?.length || !gridRef.current) {
      console.warn("[SummerMobile] Missing elements or gridRef for animation");
      return;
    }
    const { start = "top center+=100px", duration = 0.4 } = options;
  ...
  const animateElementsEnd = useCallback((ref) => {
    if (!ref?.current || !containerRef.current) {
      console.warn("[SummerMobile] Missing ref or containerRef for end animation");
      return;
    }
    gsap.to(ref.current, {
      autoAlpha: 0,
  ...
  useLayoutEffect(() => {
    if (!containerRef.current || !textRef.current || !gridRef.current) {
      console.warn("[SummerMobile] Missing required refs, skipping animations");
      return;
    }
    ...
    if (infoElements.length !== CONFIG.PLACES.length || imageElements.length !== CONFIG.PLACES.length) {
      console.warn("[SummerMobile] Mismatch in ref counts, some elements may not animate");
    }
    ...
    ctx.current = gsap.context(() => {
      ...
      if (!text) {
        console.warn("[SummerMobile] No text content for animation");
        return;
      }
      ...
      animateElements(imageElements, { start: "top center+=100px" });
