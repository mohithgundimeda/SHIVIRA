...
export default function WinterMobile() {
  const textRef = useRef(null);
  const gridRef = useRef(null);
  const containerRef = useRef(null);
  const places = useMemo(() => ['darjeeling', 'gangtok', 'manali', 'shimla', 'srinagar'], []);
  ...
  const groupedImages = useMemo(() => {
    return places.map((name) => {
      ...
      formats.forEach((form) => {
        placeObj[form] = `/static/winter/${name}/${name}-${form}/${name}-${form}-medium/${name}1.${form}`;
      });
      return placeObj;
    });
  }, [places, formats]);
